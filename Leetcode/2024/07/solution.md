...
- 책의 width, height 정보 목록과 선반의 width가 주어졌을 때 책을 순서대로 꽂았을 때 최소 높이를 구하는 문제
- 이 문제는 DP 방식으로 해결할 수 있는데요 지금까지 선반의 높이와 (이전 선반의 높이 + 지금 책의 높이) 중 더 작은 값을 계속 선반의 높이로 지정하면서 최종 선반의 높이를 구하면 됩니다. 
- 먼저 dp 배열을 books.legth + 1 크기로 만들고 마지막 위치 값을 0으로 초기화 합니다.
- 마지막 책부터 확인하면서 이전 단계 책장의 높이에 현재 책의 높이를 더합니다.
- 그리고 다음 책부터 현재 책과 다음 책을 같은 step에 위치 시킬 수 있는지 확인하면서
- 같은 step에 놓을 수 있다면 현재 책과 다음 책의 높이 중 최대 값을 height로 찾고
- width는 다음 책의 width를 더해 현재 step에 채워진 width 값을 계산합니다.
- 그리고 현재 선반의 높이와 (현재 step의 최대 책의 높이 + 이전 선반의 높이) 중 최소 값을 구해 현재 선반의 높이로 재정의 합니다.
- 이 과정을 반복해 최종적으로 dp[0]의 값을 반환하여 문제를 해결할 수 있습니다.
- 시간 복잡도: O(N^2) (N <= 1000)
