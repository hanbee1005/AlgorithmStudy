...
### 729. My Calender I ```MEDIUM```
- start, end 가 주어졌을 때 이전 일정과 비교하여 겹치지 않는 경우 true, 겹치는 경우 false 반환하는 문제
- 이 문제는 이진 탐색 트리를 구성하여 해결할 수 있습니다.
- 먼저 start, end 그리고 왼쪽 노드, 오른쪽 노드를 가지는 Node 클래스를 생성합니다.
- 이후 MyCalendar 클래스의 book 메서드에서 먼저 현재 root 노드가 null 인 경우 일정을 등록할 수 있는 것으로 간주하여 Node 클래스를 생성한 뒤, true 를 반환합니다.
- 만약 root 클래스가 존재한다면 root 노드를 현재 노드로 정하고 현재 노드의 start 보다 지금 추가해야할 end 값이 작거나 같은 경우 현재 노드의 왼쪽 노드가 비었는지 확인합니다.
- 만약 왼쪽 노드가 비어 있다면 왼쪽 노드에 지금 start, end 를 가진 Node를 만들어 추가한 뒤 true를 반환하고 비어 있지 않다면 현재 노드에 현재 노드의 왼쪽 노드를 담은 뒤 반복하여 확인합니다.
- 확인 시 현재 노드의 end 보다 start가 크거나 같은 경우에는 오른쪽 노드를 확인하여 반복하고 두 경우 모두 해당하지 않을 때는 일정을 등록할 수 없기 때문에 false를 반환하여 문제를 해결할 수 있습니다.
- 시간 복잡도: O(n) (n <= 10^9)

### 641. Design Circular Deque ```MEDIUM```
- Deque의 크키를 지정하여 생성하고 각 함수를 구현하는 문제로 생성자를 통해 Deque의 사이즈를 지정하고 프로퍼티로 가진 Deque에 데이터를 저장할 때 size와 비교한 뒤 자바에서 제공하는 Deque의 addFirst(), addLast() 등의 메서드를 사용하여 해결하였습니다.
