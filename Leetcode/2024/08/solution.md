...
- 돌이 놓여진 위치가 주어졌을 때 x가 겹치거나 y가 겹쳐서 돌을 삭제할 때 가장 많이 삭제할 수 있는 돌의 개수를 구하는 문제
- removeStones 메서드: 입력된 돌들의 위치를 이용해 행과 열을 연결합니다. 각 돌은 (row, column) 위치에 있으며, 행과 열의 연결을 위해 Union-Find 구조를 사용합니다.
parent 배열은 각 돌의 부모를 추적합니다. 크기는 20003으로 고정되어 있습니다(10001개의 행과 10001개의 열 +1).
- unionSets 메서드: 두 노드 a와 b를 연결(즉, 같은 집합에 포함)합니다. 이를 통해 행과 열을 연결합니다. 두 노드가 같은 부모를 갖지 않는다면, 하나의 부모를 다른 부모에 연결하고 섬의 수를 줄입니다.
- findParent 메서드: 경로 압축(Path Compression)을 통해 주어진 노드의 부모를 찾습니다. 이는 효율적인 Union-Find 구조를 위해 필요합니다. 부모가 없는 경우, 노드를 스스로 부모로 설정하고 섬의 수를 증가시킵니다.
- 시간 복잡도: O(N) (N <= 1000)
