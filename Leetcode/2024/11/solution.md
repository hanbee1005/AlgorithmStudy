...
- 팀의 수와 a 팀이 b 팀보다 강하다에 대한 정보가 배열로 주어졌을 때, 자신의 팀보다 강한 팀이 없는 단 하나의 팀의 번호를 구하는 문제 (없거나 2개 이상인 경우 -1)
- 먼저 팀의 개수만큼 배열을 만들고 모두 true 로 지정합니다.
- 이후 더 약한팀에 해당하는 경우 false로 변경합니다.
- 각 팀을 하나씩 확인하면서 해당 팀이 true인 경우 진적이 없는 것으로 파악해 해당 팀의 번호와 개수를 셉니다.
- 챔피온의 개수가 1인 경우 해당 챔피온 번호를, 아닌 경우 -1을 반환하여 문제를 해결할 수 있습니다.
- 시간 복잡도: O(n) (n <= 100)

### 3243. Shortest Distance After Road Addition Queries I ```MEDIUM```
- 초기 그래프에서 쿼리를 처리하며, 각 쿼리 후에 노드 0에서 노드 n−1까지의 최단 거리를 계산하여 결과로 반환하는 문제
- 초기 그래프 생성
  + ```final int[][] children = initialChildren(n, queries);```
  + initialChildren 메서드를 호출하여 초기 그래프를 생성합니다.
  + 그래프는 트리 형태이며, 각 노드는 다음 노드로의 간선을 가집니다. 즉, 0 → 1 → 2 → ... → n-1로 연결된 선형 구조입니다.
  + 이때, 각 노드의 자식 노드 목록을 저장한 2차원 배열 children을 생성합니다.
  + `numChildren` 배열: 각 노드의 자식 노드 수를 저장합니다.
    - 모든 노드는 기본적으로 하나의 자식을 가지며 (i + 1 노드), 마지막 노드 n−1은 자식이 없습니다.
    - 쿼리에서 추가될 간선의 수를 고려하여 numChildren[u] 값을 증가시킵니다.
  + `children` 배열: 각 노드의 자식 노드 목록을 저장합니다.
    - 노드 i의 첫 번째 자식은 항상 i+1 노드입니다
    - 추가 간선은 이후에 채워질 예정입니다.
- 최단 거리 배열 초기화
  + distFromZero 배열: 노드 0에서 각 노드까지의 최단 거리를 저장합니다.
  + 초기 그래프가 선형 구조이므로, 노드 i까지의 최단 거리는 i로 초기화됩니다.
- 결과 저장용 배열 및 BFS 큐 배열 생성
  + bfsPad 배열: BFS 탐색을 위한 큐 역할을 하는 배열입니다.
  + result 배열: 각 쿼리 후의 결과(노드 0에서 n−1까지의 최단 거리)를 저장합니다.
- 각 쿼리 처리
  + 간선 추가: 
    - 노드 u의 자식 노드 목록에 v를 추가합니다.
    - children[u] 배열에서 값이 0인 첫 번째 위치에 v를 넣습니다.
  + BFS 탐색을 통한 최단 거리 업데이트:
    - 시작 노드: u
    - 큐 초기화: bfsPad의 첫 번째 원소에 u를 넣습니다.
    - BFS 루프: 
      + 큐에서 노드를 하나씩 꺼내며, 인접한 자식 노드를 검사합니다.
      + 거리 업데이트 조건:
        - 기존에 저장된 거리보다 더 짧은 경로를 발견한 경우에만 업데이트합니다.
        - 이미 더 짧거나 같은 거리가 저장되어 있다면 스킵합니다.
      + 거리 업데이트 및 큐에 추가:
        - 거리를 업데이트하고, 해당 노드를 큐의 끝에 추가합니다.
  + 결과 저장:
    - 쿼리 처리 후, 노드 0에서 n−1까지의 최단 거리를 result[q]에 저장합니다.
- 시간 복잡도: O(q * n) (q <= 500, n <= 500)

### 2053. Kth Distinct String in an Array ```EASY```
- 문자열 배열에서 한번만 나온 문자 중 k번째 문자를 반환하는 문제
- 한번만 나온 문자를 담을 Set과 중복된 문자를 담을 Set을 생성합니다. 순서가 중요하기 때문에 LinkedHashSet을 사용하였습니다.
- 문자열을 확인하면서 중복되지 않은 문자열 Set에 포함되지 않고 이미 중복된 문자열 Set에도 포함되지 않은 문자를 중복되지 않은 문자열 Set에 추가합니다.
- 그렇지 않은 경우는 중복되지 않은 문자열 Set에서 제거하고 중복된 문자열 Set에 추가합니다.
- 그리고 중복되지 않은 문자열 Set의 크기보다 k가 큰 경우는 빈문자열을 그렇지 않은 경우는 중복되지 않은 문자열 Set에서 k번째에 해당하는 문자열을 찾아 반환하여 문제를 해결할 수 있습니다.
- 시간 복잡도: O(n) (n <= 1000)
