...
- 0, 1로 이루어진 이차원 배열이 주어졌을 때 1로 이루어진 정사각형의 총 개수를 구하는 문제
- DP 배열 정의
  + 주어진 이차원 배열과 동일한 크기의 dp 배열을 생성합니다. dp[i][j]는 (i, j) 위치를 오른쪽 하단으로 하는 정사각형의 한 변의 최대 길이를 나타냅니다.
- 점화식
  + 각 위치 (i, j)에 대해 matrix[i][j]가 1일 경우, dp[i][j]는 해당 위치가 정사각형의 오른쪽 하단이라고 가정할 때 만들 수 있는 최대 정사각형의 한 변의 길이가 됩니다.
  + 이를 계산하기 위해서는 dp[i-1][j], dp[i][j-1], dp[i-1][j-1]의 최소값에 1을 더한 값이 됩니다.
  + ```𝑑𝑝[𝑖][𝑗]=min(𝑑𝑝[𝑖−1][𝑗],𝑑𝑝[𝑖][𝑗−1],𝑑𝑝[𝑖−1][𝑗−1])+1 (if 𝑚𝑎𝑡𝑟𝑖𝑥[𝑖][𝑗]=1)```
- 정사각형 개수 합산
  + 각 dp[i][j] 값은 (dp[i][j] * (dp[i][j] + 1) / 2) 방식으로 해당 크기의 모든 정사각형을 포함한 개수를 누적하여 카운팅할 수 있습니다.
- 예외 처리
  + 가장자리(i==0 또는 j==0)의 경우, 인접한 위치의 최소값을 사용할 수 없으므로, matrix[i][j] == 1이면 dp[i][j] = 1로 직접 초기화합니다.
- 시간 복잡도: O(n * m) (n, m <= 300)
