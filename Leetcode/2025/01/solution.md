...
- 2 x n 배열이 주어졌을 때, 첫번째 로봇이 얻는 포인트가 최대가 된 이후 두번째 로봇이 얻을 수 있는 최대 포인트 수를 구하는 문제
- 첫번째 로봇이 첫번째 줄로 이동하고 두번째 로봇이 두번째 줄로 이동한다고 할 때, 언제 첫번째 로봇이 아래로 내려오는지에 따라 결과가 달라지게 됩니다.
- 따라서 먼저 첫번째 로봇이 움직일 첫번째 줄의 합을 구하고 
- 한칸씩 이동하면서 첫번째 로봇이 내려가기 전 값을 빼고
- 첫번째 로봇이 각 지점에서 내려갈 때 Math.max(s1, s2)로 Player 2의 점수를 계산합니다. 
  + s1은 윗줄의 남은 점수(뒤쪽 합) → 두번째 로봇이 위쪽을 지나갈 경우 점수.
  + s2는 아래줄의 앞쪽 합 → 두번째 로봇이 아래쪽을 지나갈 경우 점수.
- 그리고 모든 지점에서 두번째 로봇의 점수를 계산하며, 최소 점수를 선택합니다.
- 이렇게 모든 칸을 확인한 뒤 최소 점수를 반환하여 문제를 해결할 수 있습니다.
- 시간 복잡도: O(n) (n <= 50000)

### 1765. Map of Highest Peak ```MEDIUM```
- 0, 1로 이루어진 이차원 배열이 주어졌을 때, 1은 0으로 그리고 사방으로 높이가 1차이가 날 수 있도록 한 이차원 배열을 반환하는 문제
- 먼저 주어진 이차원 배열을 1은 0으로 0은 1로 변경합니다.
- 그리고 만약 0이 아닌 경우 '위'와 '왼쪽' 값 중 작은 값을 찾아 +1을 해서 해당 위치의 값을 변형하고
- 다시 또 0이 아닌 경우 '아래'와 '오른쪽' 값 중 작은 값을 찾아 +1을 한 뒤, 기존 값과 같이 비교해서 더 작은 값을 해당 위치의 값으로 변형합니다.
- 이렇게 모든 위치를 확인한 뒤 반환하여 문제를 해결할 수 있습니다.
- BFS로 0인 위치를 큐에 추가하고 전체 응답을 -1로 변경한 뒤, 하나씩 큐에서 꺼내면서 -1이 아닌 경우 이전 위치 값 + 1 을 해서 해당 위치의 값을 변경하고 상하좌우를 확인하면서 -1이 아닌 경우 해당 위치와 이전 값 + 1을 해서 큐에 넣는 방식으로도 해결할 수 있습니다.
- 시간 복잡도: O(n * m) (n, m <= 1000)

### 9. Palindrome Number ```EASY```
- 정수가 주어졌을 때 뒤집어서 같은 수이면 true, 다르면 false 를 반환하는 문제
- 음수인 경우에는 반대로하면 다른 수이기 때문에 false를 바로 반환합니다.
- x 를 10으로 나눠서 나머지를 구하고 원래 값에 10을 곱한 뒤 뒤에 나머지를 더하는 과정을 반복하여 주어진 x를 뒤집고
- 결과적으로 x와 뒤집은 값을 비교하여 같으면 true, 다르면 false 를 반환하여 문제를 해결할 수 있습니다.
- 시간 복잡도: O(n) (n <= 20)

### 14. Longest Common Prefix ```EASY```
- 문자열 배열이 주어졌을 때, 중복된 길이의 가장 긴 prefix를 구하는 문제
- 배열이 비어있는 경우 빈 문자열을 바로 반환합니다.
- 첫번째 글자 전부를 prefix로 저장하고 문자열 하나씩 돌아가면서 해당 문자열에서 prefix를 포함하는 인덱스가 0이 아닌 경우 (즉, prefix로 시작하지 않는 경우)
- prefix를 한글자씩 줄이면서 (뒷 문자부터 제거) 해당하는 prefix를 찾고
- 최종적으로 prefix를 반환하여 문제를 해결할 수 있습니다.
- 시간 복잡도: O(n * m) (n <= 200, m <= 200)
