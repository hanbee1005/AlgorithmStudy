...
- {key, value}로 이루어진 이차원 배열이 2개 주어졌을 때, key가 같은 경우 value를 합하여 오름차순으로 정렬된 배열을 반환하는 문제
- 먼저 각각의 위치를 나타낼 i, j 변수를 지정한 뒤, i가 nums1의 길이보다 작고 j가 nums2의 길이보다 작은 동안
- nums1의 i번째 키와 nums2의 j번째 키가 같은지 확인하고 같다면 두 value를 합하여 list에 저장합니다.
- nums1의 i번째 키가 nums2의 j번째 키보다 작은 경우는 nums1의 i번째 키, 값만 리스트에 추가하고
- nums1의 i번째 키가 nums2의 j번째 키보다 큰 경우는 nums2의 j번째 키, 값만 리스트에 추가합니다.
- 그리고 nums1과 nums2 둘다 끝까지 확인하면서 key, value를 결과 list에 저장한 뒤, 해당 리스트를 배열로 변경하여 반환하면 문제를 해결할 수 있습니다.
- 시간 복잡도: O(N) (N <= 200)

### 2033. Minimum Operations to Make a Uni-Value Grid ```MEDIUM```
- 2차원 배열과 x가 주어졌을 때, 2차원 배열의 각 값에 x를 더하거나 빼서 모두 같은 숫자를 만들 때 드는 최소 연산의 수를 구하는 문제 (없는 경우 -1 반환)
- 각 숫자를 확인하면서 동일 숫자의 개수를 count 배열에 저장합니다.
- count 배열을 순회하면서 중간 숫자를 구합니다.
- 0부터 중간 숫자까지는 중간 숫자에서 해당 값을 빼고 x로 나눈 수(연산의 개수) * 해당 값의 개수(count)를 해서 총 연산 수에 더합니다.
- 중간 숫자 이후부터 마지막 숫자까지는 해당 값에서 중간 숫자를 빼고 x로 나눈 수(연산의 수) * 해당 값의 개수(count)를 해서 총 연산 수에 더합니다.
- 최종적으로 총 연산 수를 반환하여 문제를 해결할 수 있습니다. 
- 시간 복잡도: O(N * M) (N * M <= 100000)
