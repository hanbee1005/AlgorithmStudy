...
- Alice가 최적의 리프 노드로 이동할 때 얻을 수 있는 최대 순이익을 구하는 문제
  + n개의 노드(0부터 n-1까지 레이블된)로 구성된 루트가 0인 무방향 트리가 주어집니다. 
  + 각 노드 i에는 게이트가 있으며, amount[i] 배열은 해당 노드의 게이트를 여는 데 필요한 비용(음수) 또는 열었을 때 얻는 보상(양수)을 나타냅니다. 
  + Alice는 노드 0에서 시작하여 리프 노드로 이동하고, Bob은 노드 bob에서 시작하여 노드 0으로 이동합니다. 
  + 두 사람이 동시에 같은 노드에 도달하면, 그 노드의 비용이나 보상을 반씩 나눕니다.
- 트리 구성: 주어진 간선 정보를 통해 트리를 구성합니다.
- Bob의 경로 추적: Bob이 노드 0에 도달할 때까지의 경로와 각 노드에 도달하는 시간을 기록합니다.
- 이익 계산: Alice가 DFS를 통해 리프 노드까지 이동하면서, 각 노드에서 Bob의 도착 시간과 비교하여 Alice의 순이익을 계산합니다. 동시에 두 사람이 도착하면 보상을 반으로 나누고, Bob이 먼저 도착하면 보상을 받을 수 없습니다.
- 시간 복잡도: O(n) (n <= 100000)

### 1749. Maximum Absolute Sum of Any Subarray ```MEDIUM```
- 주어진 정수 배열 nums에서 절대값이 가장 큰 부분 배열의 합을 찾는 문제
- 배열 nums에서 부분 배열의 합을 계산할 때, 그 절대값이 가장 큰 값을 찾는 것이 목표입니다.
- 즉, 부분 배열의 합이 최대가 되는 경우와 최소가 되는 경우를 찾아서, 이 둘의 절대값이 가장 큰 경우를 반환하면 됩니다.
- 이 방법은 누적합(prefix sum)과 최댓값/최솟값 추적을 이용하는 접근법입니다.
  + 특정 부분 배열 [i, j]의 합을 sum[j] - sum[i-1]로 표현할 수 있습니다.
  + 최대 절대값을 가지는 부분 배열은 sum[j] - sum[i]가 가장 크거나 가장 작은 경우를 찾으면 됩니다.
  + 즉, 누적합의 최댓값과 최솟값의 차이를 구하면 정답이 됨을 알 수 있습니다.
