...
- 수학적인 공식을 통해서도 문제를 해결할 수 있습니다.
  ...
  ### Top Coder level 2. 무한 수열
  - 이 문제는 n, p, q, x, y 가 주어졌을 때 n 번째 수를 구하는 문제로 n이 0 이하일 때는 1, 양수일 때는 (n / p - x) 번째 수 + (n / q - y) 번째 수 값을 구하는 문제입니다.
  - 이 문제는 수의 범위가 커서 메모리 낭비가 있을 수 있기 때문에 적당히 작은 수와 큰 수로 나누어서 처리를 해야 합니다.
  - 적당히의 기준을 10의 6승 수로 두고 그 이하인 경우는 배열에 담에서 같은 수를 여러번 계산하지 않도록 하고
  - 적당 수 이상인 경우는 그때 그때 계산된 값을 사용할 수 있도록 합니다.
  - 재귀 함수 + 동적 계획법을 사용하여 문제를 해결할 수 있습니다.

  ### Top Coder level 3. 바닥 타일
  - '.'과 '#'으로 이루어진 바닥이 주어졌을 때 가로가 1이고 세로가 n인 타일로 바닥을 채우는 최소 타일 수를 구하는 문제입니다.
  - 이 문제는 비트 연산과 동적 계획법을 사용하여 해결할 수 있습니다.
  - (해설을 이해하지 못했습니다... ㅠㅠ)