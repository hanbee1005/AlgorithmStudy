...
### level 3. 양과 늑대
- 각 노드가 양(0)인지 늑대(1)인지를 나타내는 배열과 각 배열의 연결 관계(부모, 자식)가 배열로 주어졌을 때 잡아 먹히지 않고 찾을 수 있는 양의 최대 수를 구하는 문제
- ```List<List<Integer>>``` 형태로 각 노드와 연결된 노드를 저장합니다.
- ```DFS(현재 노드 번호, 양의 수, 늑대의 수, 다음으로 방문할 수 있는 노드의 집합)``` 을 통해 구할 수 있습니다.
- 먼저 현재 노드의 값이 양인지 늑대인지에 따라 지금까지 찾은 양 또는 늑대의 수를 증가시킵니다.
- 그리고 만약 늑대의 수가 양의 수보다 크거나 같다면 더이상 해당 노드의 하위를 탐색할 수 없기 때문에 return 합니다.
- 양의 수가 더 많다면 현재까지의 양의 수와 지금까지 양의 최대 수 중 더 큰 수를 양의 최대 수로 갱신합니다.
- 그리고 이전에서 넘어온 다음 방문할 수 있는 노드에서 현재 탐색한 노드를 제외한 뒤, 현재 노드의 하위 노드 목록을 추가한 다음 해당 노드들을 다시 반복하여 탐색합니다.
- 이후 최종적으로 최대 양의 수를 반환하여 문제를 해결할 수 있습니다.
- 참고: https://tech.kakao.com/posts/488
- 시간 복잡도: O(N^2) (2 <= N <= 17)

### level 3. 기둥과 보 설치
- 전체 맵의 크기와 {x, y, 기둥 또는 보, 설치 또는 삭제} 정보가 배열로 주어졌을 때 최종적으로 세워진 기둥과 보의 위치와 타입을 배열로 반환하는 문제
- 구현 문제이기 때문에 하나씩 실제 구현을 하고 삭제하는 조건은 체크할 것이 많기 때문에 일단 삭제를 한 뒤 그 다음에도 전체적으로 구조가 가능하다면 삭제하고 아니면 다시 돌려놓는 방식으로 구현하였습니다.
- 먼저 기둥이 세워진 위치와 보가 세워진 위치를 담은 이차원 배열을 2개 만듭니다.
- 기둥을 세울 경우 바닥인지(```y == 0```) 이미 기둥이 세워져 있는지(```y > 0 && pillar[x][y-1]```) 아니면 보의 시작이나 끝에 해당하는지(```x > 0 && bar[x-1][y] || bar[x][y]```) 확인하고 이때 ```pillar[x][y]```를 true로 바꿔 기둥을 설치합니다.
- 보를 설치하는 경우에는 한쪽 끝에 기둥이 있는 경우(```y > 0 && pillar[x][y-1] || pillar[x+1][y-1]```) 아니면 양쪽 끝에 보가 연결되어 있는지(```x > 0 && bar[x-1][y] && bar[x+1][y]```) 확인하고 이때 ```bar[x][y]```를 true로 바꿔 보를 설치합니다.
- 기둥이나 보를 삭제하는 경우에는 해당 위치의 기둥이나 보를 우선 삭제한 뒤 전체 맵을 확인하면서 기둥이 세워져 있는데 기둥을 세울 수 없는 경우 보가 설치되어 있는데 해당 위치에 보를 설치할 수 없는 경우 다시 삭제한 기둥이나 보를 복원하는 방식으로 처리합니다.
- 기둥이나 보를 설치하거나 제거할 때 설치된 총 개수를 미리 계산해 뒀다가 반환할 int 배열을 생성하고
- 전체 맵을 반복하면서 해당 위치에 기둥이 있거나 보가 있는 경우 계속 추가한 뒤 최종 결과를 반환하여 문제를 해결할 수 있습니다.
- 시간 복잡도: (N^2 * M) (N <= 100, M <= 1000)

### level 3. 보행자 천국
- 전체 도로의 크기 m, n과 도로의 상황이 배열로 주어졌을 때 왼쪽 상단에서 오른쪽 하단으로 갈 수 있는 모든 경로의 수를 구하는 문제
- 차는 왼쪽에서 오른쪽으로, 위에서 아래로만 움직일 수 있습니다.
- dp 방식으로 문제를 해결할 수 있는데 먼저 x,y 위치에 왼쪽에서 온 경로의 수, 위에서 온 경로의 수를 담는 3차원 배열을 생성합니다.
- 그리고 제일 상단 (0,y)와 제일 왼쪽 (x,0)을 먼저 확인하여 각 위치 경로를 초기화 합니다.
- 이후 x=1, y=1부터 반복하면서 현재 위치보다 위쪽의 수가 0인 경우 위쪽 위치의 왼쪽 경로 수와 위쪽 경로 수를 더한 값을 현재 위치의 위쪽 경로 수에 저장합니다.
- 만약 현재 위치보다 위쪽의 수가 2인 경우에는 위쪽 위치의 위쪽 경로수만 현재 위치의 위쪽 경로 수에 저장합니다.
- 다음으로 현재 위치보다 왼쪽 위치의 수가 0인 경우 왼쪽 위치의 왼쪽 경로 수와 위쪽 경로 수를 모두 더해 현재 위치의 왼쪽 경로 수에 저장하고
- 현재 위치보다 왼쪽 위치의 수가 2인 경우 왼쪽 위치의 왼쪽 경로 수만 현재 위치의 왼쪽 경로 수에 저장합니다.
- 이후 마지막 오른쪽 하단 위치의 왼쪽 경로 수와 위쪽 경로 수의 합을 반환하여 문제를 해결할 수 있습니다.
- 시간 복잡도: O(m * n) (m,n <= 500)

### level 3. 광고 삽입
- 전체 영상 재생 시간, 광고 재생 시간, 시청자들의 영상 시청 시작 시간과 종료 시간이 배열로 주어졌을 때 광고 시청 시간이 최대가 될 수 있는 시작 시간을 구하는 문제
- 이 문제는 같은 구간을 여러번 더해야하는 문제로 누적합을 사용하여 해결할 수 있습니다.
- 먼저 영상 재생 시간과 광고 재생 시간이 동일한 경우 바로 "00:00:00"을 반환합니다.
- 영상 최대 시간인 36000 + 1 크기의 시간 배열을 생성한 뒤 영상의 시작 시간에 1을 종료 시간에 -1을 저장합니다. (원래 누적 합은 end+1에 -1을 추가해야하지만 영상 종료 시간은 포함하지 않기 때문에 -1을 종료 시간에 추가합니다.)
- 그리고 다시 1부터 시간 배열을 반복하면서 현재 시간 값에 바로 앞 시간 값을 더해줍니다.
- 그리고 나서 0부터 영상 재생 시간에서 광고 재생 시간을 뺀 시간까지 반복하면서 (현 시점 + 광고 재생 시간) 시간의 누적 시청 시간에서 현재 시점의 누적 시청 시간을 뺀 값을 현재 누적 시청 시간에 추가하고 만약 최대 누적 시청 시간보다 큰 경우 최대 값을 갱신한 뒤, 현재 시점 + 1한 시간을 시작 시간으로 지정합니다.
  + 10분부터 20분동안 광고를 재생하는 경우 (30분 시점의 누적 합 - 10분 시점의 누적 합)을 하면 현재 누적 시간이 나오게 되고
  + 시작 시간을 저장할 때는 10분이 아니라 11분을 저장하게 되는데 이유는 10분 00초 01, 02... 등을 10분 01초로 간주하는 것 같습니다. ㅠ
- 모든 시간을 확인한 뒤 최종적으로 시작 시간을 반환하여 문제를 해결할 수 있습니다. 
- 시간 복잡도: O(N) (N <= 360001)

### level 3. 미로 탈출 명령어
- 미로의 크기 n, m, 시작 위치 x, y, 도착 위치 r, c, 이동 수 k 가 주어졌을 때 도달할 수 있는 경로 중 사전적 우선 순위가 가장 앞서는 경로를 구하는 문제
- 이 문제를 DFS로 해결하려고 했으나 시간 초과로 실패하였고 정해진 k 수를 모두 쓰면서 오름차순을 유지해야 하기 때문에 가능하면 dl 방향으로 최대한 이동한 뒤, 다시 ru 쪽으로 이동하는 방식을 사용해야 합니다.
- 먼저 이동해야하는 길이가 k 보다 크다면 이동이 불가하므로 "impossible"을 바로 반환합니다.
- 그리고 k의 길이를 줄여가면서 먼저 아래 방향으로 이동할 수 있는지 확인합니다. 다음 아래 방향(x+1)이 범위 안에 있고 해당 위치로 이동할 때 거리(```Math.abs(r - downX) + Math.abs(c - y)```)가 현재 남은 거리인 k 보다 작거나 같다면 이동할 수 있는 것으로 간주하고 경로에 d 문자를 더한 뒤, 아래 방향으로 이동합니다. (x 값 갱신)
- 이런 방식으로 d -> l -> r -> u 순으로 검사를 진행하고 최종적으로 경로를 반환하여 문제를 해결할 수 있습니다.
- 시간 복잡도: O(N) (N <= 2500)
