...
### Level 2. 리코쳇 로봇
- 로봇의 위치 R, 장애물 위치 D, 도착 위치 G 가 표시된 게임판이 주어졌을 때 G까지 도착하는 가장 빠른 방법을 구하는 문제입니다.
- 로봇이 이동할 때는 한 방향으로 D 또는 끝에 도달할 때까지 이동하는 것을 1번 이동으로 생각합니다.
- 이 문제는 BFS 를 통해 문제를 해결할 수 있는데
- 다음 위치를 구할 때 D 또는 끝에 도달하기 전 위치를 찾아 해당 위치를 방문 했는지 등으로 확인하여 해당 위치까지 계속 이동하는 방식으로
- 최종적으로 G를 찾으면 해결할 수 있는 문제입니다.